...
int bcast_warp(int arg, const sycl::nd_item<3> &item_ct1,
...
*/
    /*
DPCT1096:14: The right-most dimension of the work-group used in the SYCL kernel
...
*/
    value = dpct::select_from_sub_group(
        item_ct1.get_sub_group(), value,
        0); // Synchronize all threads in warp, and get "value" from lane 0
    if (value != arg && laneId == 0)
        /*
DPCT1015:5: Output needs adjustment.
*/
        stream_ct1 << "Thread %d failed. with val:%d, arg:%d \n";
    return value;
}

void print_mer(cstr_type& mer, const sycl::stream &stream_ct1){
   // if(threadIdx.x%32 == 0){
    for(int i = 0; i < mer.length; i++){
        /*
DPCT1015:6: Output needs adjustment.
*/
        stream_ct1 << "%c";
    }
    stream_ct1 << "\n";
...
void cstr_copy(cstr_type& str1, cstr_type& str2){
...
unsigned hash_func(cstr_type key, uint32_t max_size){
    unsigned hash, i;
...
void ht_insert(loc_ht* thread_ht, cstr_type kmer_key, MerFreqs mer_val, uint32_t max_size){
...
void ht_insert(loc_ht_bool* thread_ht, cstr_type kmer_key, bool bool_val, uint32_t max_size){
    unsigned hash_val = hash_func(kmer_key, max_size);
    //int count = 0; // for debugging
    while(true){
        int if_empty = thread_ht[hash_val].key.length; // length is set to some unimaginable number to indicate if its empty
...
char walk_mers(loc_ht* thrd_loc_ht, loc_ht_bool* thrd_ht_bool, uint32_t max_ht_size, int& mer_len, cstr_type& mer_walk_temp, cstr_type& longest_walk, cstr_type& walk, const int idx, int max_walk_len,
    ...
    for( int nsteps = 0; nsteps < max_walk_len; nsteps++){
        ...
        loc_ht_bool &temp_mer_loop =
            ht_get(thrd_ht_bool, mer_walk_temp, max_walk_len, stream_ct1);
        ...
        loc_ht &temp_mer = ht_get(thrd_loc_ht, mer_walk_temp, max_ht_size, stream_ct1);
...
const sycl::nd_item<3> &item_ct1, const sycl::stream &stream_ct1){
    ...
    for(int i = 0; i < r_rds_cnt; i++){
        ...
        for( int start = lane_id; start < num_mers; start+=32){
          ...
          if(ext_pos >= (int) read.length)
            stream_ct1 << "*********ASSERTION FAILURE IN COUNT_MERS****";
    ...
    for(auto k = lane_id; k < max_ht_size; k+=32){
        if(thrd_loc_ht[k].key.length != EMPTY){
            thrd_loc_ht[k].val.set_ext(loc_ctg_depth, stream_ct1);
        }
    }
    sycl::group_barrier(item_ct1.get_sub_group());
    int test = 1;
...
iterative_walks_kernel exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL void iterative_walks_kernel(
    uint32_t *cid, uint32_t *ctg_offsets, char *contigs, char *reads_r,
    char *quals_r, uint32_t *reads_r_offset, uint32_t *rds_count_r_sum,
    double *ctg_depth, loc_ht *global_ht, uint32_t *prefix_ht,
    loc_ht_bool *global_ht_bool, int kmer_len, uint32_t max_mer_len_off,
    uint32_t *term_counts, int64_t num_walks, int64_t max_walk_len,
    int64_t sum_ext, int32_t max_read_size, int32_t max_read_count,
    uint32_t qual_offset, char *longest_walks, char *mer_walk_temp,
    uint32_t *final_walk_lens, int tot_ctgs, const sycl::nd_item<3> &item_ct1,
    const sycl::stream &stream_ct1)
{
    const long int idx = item_ct1.get_local_id(2) +
                         item_ct1.get_group(2) * item_ct1.get_local_range(2);
    const long int warp_id_glb = idx/32;
    const long int lane_id = item_ct1.get_local_id(2) % 32;
    ...
    if(warp_id_glb < tot_ctgs){ // all warps within this range can go in execute count mers, for walk_mers only the lane 0 of each warp does the work
            ...
            count_mers(loc_mer_map, loc_r_reads, max_ht_size, loc_r_quals,
                       ...
                       loc_ctg_depth, mer_len, qual_offset, excess_reads,
                       warp_id_glb, item_ct1,
                       stream_ct1); // passing warp_id instead of idx now
        ...
        if(lane_id == 0){ // this phase is processed by single thread of a warp
            ...
            char walk_res =
                walk_mers(loc_mer_map, loc_bool_map, max_ht_size, mer_len,
                          loc_mer_walk, longest_walk_thread, walk, warp_id_glb,
                          max_walk_len, stream_ct1);
            // #ifdef DEBUG_PRINT_GPU
            // if(warp_id_glb == test){
        ...
        sycl::group_barrier(item_ct1.get_sub_group());
        /*
DPCT1086:9: __activemask() is migrated to 0xffffffff. You may need to adjust the
code.
*/
        ...
        shift = bcast_warp(shift, item_ct1, stream_ct1);
        }//warp id cond end
