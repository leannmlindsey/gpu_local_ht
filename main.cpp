...
size_t get_device_mem(){
    ...
    gpus = dpct::dev_mgr::instance().device_count();
    for ( int id = 0; id < gpus; id++ ) {
        /*
DPCT1093:11: The "id" device may be not the one intended for use. Adjust the
selected device if needed.
*/
        dpct::select_device(id);
        /*
DPCT1106:12: 'cudaMemGetInfo' was migrated with the Intel extensions for device
...
//allocating maximum possible memory for a single iteration
    ...
    //allocate GPU  memory
    CUDA_CHECK(
        DPCT_CHECK_ERROR(prefix_ht_size_d = sycl::malloc_device<uint32_t>(
                             slice_size, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(cid_d = sycl::malloc_device<uint32_t>(
                                    slice_size, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(ctg_seq_offsets_d = sycl::malloc_device<uint32_t>(
                             slice_size, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        reads_l_offset_d = sycl::malloc_device<uint32_t>(
            max_l_rds_its,
            dpct::get_default_queue()))); // changed this with new max
    CUDA_CHECK(DPCT_CHECK_ERROR(
        reads_r_offset_d = sycl::malloc_device<uint32_t>(
            max_r_rds_its,
            dpct::get_default_queue()))); // changed this with new max
    CUDA_CHECK(
        DPCT_CHECK_ERROR(rds_l_cnt_offset_d = sycl::malloc_device<uint32_t>(
                             slice_size, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(rds_r_cnt_offset_d = sycl::malloc_device<uint32_t>(
                             slice_size, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        ctg_seqs_d = sycl::malloc_device<char>(
            max_ctg_len_it,
            dpct::get_default_queue()))); // changed this with new max
    CUDA_CHECK(
        DPCT_CHECK_ERROR(reads_left_d = (char *)sycl::malloc_device(
                             sizeof(char) * max_read_size * max_l_rds_its,
                             dpct::get_default_queue()))); // changed
    CUDA_CHECK(
        DPCT_CHECK_ERROR(reads_right_d = (char *)sycl::malloc_device(
                             sizeof(char) * max_read_size * max_r_rds_its,
                             dpct::get_default_queue()))); // changed
    CUDA_CHECK(DPCT_CHECK_ERROR(depth_d = sycl::malloc_device<double>(
                                    slice_size, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(quals_right_d = (char *)sycl::malloc_device(
                             sizeof(char) * max_read_size * max_r_rds_its,
                             dpct::get_default_queue()))); // changed this
    CUDA_CHECK(DPCT_CHECK_ERROR(
        quals_left_d = (char *)sycl::malloc_device(
            sizeof(char) * max_read_size * max_l_rds_its,
            dpct::get_default_queue()))); // changed this with new
    CUDA_CHECK(DPCT_CHECK_ERROR(term_counts_d = sycl::malloc_device<uint32_t>(
                                    3, dpct::get_default_queue())));
    ...
    CUDA_CHECK(DPCT_CHECK_ERROR(
        d_ht = sycl::malloc_device<loc_ht>(
            max_ht,
            dpct::get_default_queue()))); //**changed for new modifications
    CUDA_CHECK(DPCT_CHECK_ERROR(longest_walks_d = (char *)sycl::malloc_device(
                                    sizeof(char) * slice_size * max_walk_len,
                                    dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        mer_walk_temp_d = (char *)sycl::malloc_device(
            (max_mer_len + max_walk_len) * sizeof(char) * slice_size,
            dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(d_ht_bool = (loc_ht_bool *)sycl::malloc_device(
                             sizeof(loc_ht_bool) * slice_size * max_walk_len,
                             dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(final_walk_lens_d = sycl::malloc_device<uint32_t>(
                             slice_size, dpct::get_default_queue())));
    mem_timer.timer_end();
...
//calls the kernels, revcomps, copy backs walks and
    ...
    for(int slice = 0; slice < iterations; slice++){
        ...
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_default_queue()
                                        .memcpy(prefix_ht_size_d,
                                                prefix_ht_size_h.get(),
                                                sizeof(uint32_t) * vec_size)
                                        .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(cid_d, cid_h.get(), sizeof(uint32_t) * vec_size)
                .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_default_queue()
                                        .memcpy(ctg_seq_offsets_d,
                                                ctg_seq_offsets_h.get(),
                                                sizeof(uint32_t) * vec_size)
                                        .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(reads_l_offset_d, reads_l_offset_h.get(),
                        sizeof(uint32_t) * total_l_reads_slice)
                .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(reads_r_offset_d, reads_r_offset_h.get(),
                        sizeof(uint32_t) * total_r_reads_slice)
                .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_default_queue()
                                        .memcpy(rds_l_cnt_offset_d,
                                                rds_l_cnt_offset_h.get(),
                                                sizeof(uint32_t) * vec_size)
                                        .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_default_queue()
                                        .memcpy(rds_r_cnt_offset_d,
                                                rds_r_cnt_offset_h.get(),
                                                sizeof(uint32_t) * vec_size)
                                        .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_default_queue()
                                        .memcpy(ctg_seqs_d, ctg_seqs_h.get(),
                                                sizeof(char) * ctgs_offset_sum)
                                        .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(reads_left_d, reads_left_h.get(),
                                         sizeof(char) * reads_l_offset_sum)
                                 .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(reads_right_d, reads_right_h.get(),
                                         sizeof(char) * reads_r_offset_sum)
                                 .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(depth_d, depth_h.get(), sizeof(double) * vec_size)
                .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(quals_right_d, quals_right_h.get(),
                                         sizeof(char) * reads_r_offset_sum)
                                 .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(quals_left_d, quals_left_h.get(),
                                         sizeof(char) * reads_l_offset_sum)
                                 .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(term_counts_d, term_counts_h.get(),
                                         sizeof(uint32_t) * 3)
                                 .wait()));
        tim_temp.timer_end();
	...
	unsigned thread_per_blk = 256; //TODO max_work_group_size
...
*/
            ...
            dpct::get_default_queue().submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(
                    sycl::nd_range<3>(sycl::range<3>(1, 1, blocks) *
                                          sycl::range<3>(1, 1, thread_per_blk),
                                      sycl::range<3>(1, 1, thread_per_blk)),
                    [=](sycl::nd_item<3> item_ct1)
                        [[intel::reqd_sub_group_size(32)]] {
                            iterative_walks_kernel(
                                cid_d, ctg_seq_offsets_d, ctg_seqs_d,
                                reads_right_d, quals_right_d, reads_r_offset_d,
                                rds_r_cnt_offset_d, depth_d, d_ht,
                                prefix_ht_size_d, d_ht_bool, mer_len,
                                max_mer_len, term_counts_d, num_walks,
                                max_walk_len, sum_ext, max_read_size,
                                max_read_count, qual_offset, longest_walks_d,
       ...
       // rev_comp_.timer_start();
        ...
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(longest_walks_r_h.get() +
                            slice * max_walk_len * slice_size,
                        longest_walks_d, sizeof(char) * vec_size * max_walk_len)
                .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(final_walk_lens_r_h.get() + slice * slice_size,
                        final_walk_lens_d, sizeof(int32_t) * vec_size)
                .wait()));

        //cpying rev comped ctgs to device on same memory as previous ctgs
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_default_queue()
                                 .memcpy(ctg_seqs_d, ctgs_seqs_rc_h.get(),
                                         sizeof(char) * ctgs_offset_sum)
                                 .wait()));
        tim_temp.timer_end();
        data_mv_tim += tim_temp.get_total_time();
        /*
DPCT1049:3: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
        {
                ...
                cgh.parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1)
                        [[intel::reqd_sub_group_size(32)]] {
                            iterative_walks_kernel(
                                cid_d, ctg_seq_offsets_d, ctg_seqs_d,
                                reads_left_d, quals_left_d, reads_l_offset_d,
                                rds_l_cnt_offset_d, depth_d, d_ht,
                                prefix_ht_size_d, d_ht_bool, mer_len,
                                max_mer_len, term_counts_d, num_walks,
                                max_walk_len, sum_ext, max_read_size,
                                max_read_count, qual_offset, longest_walks_d,
        ...
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(longest_walks_l_h.get() +
                            slice * max_walk_len * slice_size,
                        longest_walks_d, sizeof(char) * vec_size * max_walk_len)
                .wait())); // copy back left walks
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_default_queue()
                .memcpy(final_walk_lens_l_h.get() + slice * slice_size,
                        final_walk_lens_d, sizeof(int32_t) * vec_size)
                .wait()));
        tim_temp.timer_end();
        data_mv_tim += tim_temp.get_total_time();
...
//once all the alignments are on cpu, then go through them and stitch them with contigs in front and back.
  ...
  for(int j = 0; j < iterations; j++){
    ...
    mem_timer.timer_start();
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(term_counts_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(cid_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(ctg_seq_offsets_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(reads_l_offset_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(reads_r_offset_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(rds_l_cnt_offset_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(rds_r_cnt_offset_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(ctg_seqs_d, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(reads_left_d, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(reads_right_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(depth_d, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(quals_right_d, dpct::get_default_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(quals_left_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(d_ht, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(longest_walks_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(mer_walk_temp_d, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(d_ht_bool, dpct::get_default_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(final_walk_lens_d, dpct::get_default_queue())));
    mem_timer.timer_end();
