...
do
    ...
    ./test_script_benchmark_large.sh > "$OUTPUT_FILE"
    echo "Test $i complete, results stored in $OUTPUT_FILE"
done

echo "All tests completed."

echo "Average of 10 tests on large dataset, kmer=21"
cat results* | grep 'time for 21' | awk '{sum+=$NF} END {print sum/NR}'
echo "Average of 10 tests on large dataset, kmer=33"
cat results* | grep 'time for 33' | awk '{sum+=$NF} END {print sum/NR}'
echo "Average of 10 tests on large dataset, kmer=55"
cat results* | grep 'time for 55' | awk '{sum+=$NF} END {print sum/NR}'
echo "Average of 10 tests on large dataset, kmer=77"
