...
struct cstr_type{
    ...
    bool operator==(const cstr_type& in2){
        bool str_eq = true;
...
*/
    stream_ct1 << "count_A:%d, count_C:%d, count_G:%d, count_T:%d\n";
  }
...
*/
      stream_ct1 << "base:%c, nvotes_hiq_q:%d, nvotes:%d, rating:%d\n";
    }

    
    uint16_t get_base_rating(int depth) {
      double min_viable = sycl::max(LASSM_MIN_VIABLE_DEPTH * depth, 2.0);
      double min_expected_depth = sycl::max(LASSM_MIN_EXPECTED_DEPTH * depth, 2.0);
...
struct MerFreqs {
  ...
  void set_ext(int seq_depth, const sycl::stream &stream_ct1) {
   ...
   // assert(top_rating >= runner_up_rating);// for now finding a way around for assertion
    if(top_rating < runner_up_rating)
      stream_ct1 << "******* ASSERTION FAILED IN sort_merbase************";
...
struct loc_ht{
    cstr_type key;
    MerFreqs val;
    loc_ht(cstr_type in_key, MerFreqs in_val){
      key = in_key;
      val = in_val;
    }
};
...
struct loc_ht_bool{
    cstr_type key;
    bool val;
    loc_ht_bool(cstr_type in_key, bool in_val){
      key = in_key;
      val = in_val;
    }
};
...
void ht_kernel(loc_ht* ht, char* contigs, int* offset_sum, int kmer_size);
void ht_insert(loc_ht* thread_ht, cstr_type kmer_key, cstr_type ctg_val, uint32_t max_size);
